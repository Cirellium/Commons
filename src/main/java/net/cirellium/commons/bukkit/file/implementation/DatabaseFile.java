...
/**
 * The implementation of the {@link AbstractPluginFile} class for database files.
...
public class DatabaseFile<P extends CirelliumBukkitPlugin<P>> extends AbstractPluginFile<P> {
    ...
    /**
     * ! Code smell and not object oriented, but Java doesn't allow switching over different types without using tons of if-elses and instanceofs.
     * 
     * If possible, converts a given object to a string and sets the value at a given path.
     * 
     * @param path The path to set the value at.
     * @param value The value to set.
     */
     ...
     * @return A {@link Location} from the world, x, y, z, yaw and pitch values at the given path.
     */
    ...
    /**
     * Gets the value of a given path as an {@link Inventory} and fills it with the items from the given path.
    ...
    public Inventory getInventory(String path, int size, String title) {
        Inventory inventory = Bukkit.createInventory(null, size, Component.text(ChatColor.translateAlternateColorCodes('&', title)));
    ...
    /**
     * Gets all the keys from the {@code fileConfig} as a {@link String} and returns them as a {@link Set}.
    ...
    /**
     * Gets all the sub-keys of a given path as a {@link String} and returns them as a {@link Set}.
